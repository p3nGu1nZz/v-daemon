...
config_init() {
  ...
  if [ -f "$CONFIG_FILE" ]; then
    ...
    director_timeout_val="$(_toml_get director copilot_timeout_seconds "$CONFIG_FILE")"
    if [ -n "$director_timeout_val" ]; then
      DIRECTOR_COPILOT_TIMEOUT_SECONDS="$director_timeout_val"
    else
      DIRECTOR_COPILOT_TIMEOUT_SECONDS="${DIRECTOR_COPILOT_TIMEOUT_SECONDS:-300}"
    fi
  fi
  ...
  export REPO_ROOT RUN_DIR LOG_DIR DAEMON_PIDFILE SUP_PIDFILE DIRECTOR_PIDFILE DAEMON_LOG SUP_LOGFILE DIRECTOR_LOG HEARTBEAT_INTERVAL DIRECTOR_ALLOW_EXECUTE DIRECTOR_SANDBOX_BRANCH_PREFIX DIRECTOR_INTERVAL_SECONDS DIRECTOR_COPILOT_TIMEOUT_SECONDS
}
